...
from api import getUser, updateUsers, setLocation, takeOrder, getPayment, updateTask, countProgress  
...
# bot.set_my_commands([('start', 'Запустить бота')])

@bot.message_handler(commands=['start'])
def start(message):
    user = getUser(message.from_user.id)
    state.resetState()
    count = countProgress(message.from_user.id)
    state.setProgress(count)
    if (len(user) == 0):
        updateUsers(message)
        ...
        page = mainPage(message.from_user.username,
                        user[0][0], user[0][2], user[0][1], state.getState()['progress'])
...
def handleText(message):
    ...
    if (state.getState()['in_tasks']):
        ...
        updateTask(message.text, orderId, message.from_user.id)
        takeOrder(orderId)
